...
interface LabelDao {
    ...
    @Query(
        "SELECT label.* FROM ${EventLabelEntity.TABLE_NAME} as eventLabel " +
                "JOIN ${LabelEntity.TABLE_NAME} AS label ON eventLabel.labelId = label.id " +
                "WHERE eventLabel.eventId = :eventId"
    )
